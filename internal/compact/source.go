...
// DayActivity is one day's headline volume. It answers what history exists and
// how dense it is, which is a different question from what a window totals to:
// a run of empty days between two busy ones is a fact about how the agent was
// used, and no aggregate can show it.
...
// Activity returns per-day volume for the given days, in the order given.
//
// It reads the same rollups the dashboard does — the consolidated index first,
// then the day's own partition — so the cost is one row per day. Only a day
// with neither, which in practice means today, falls back to counting turns.
func (c *Compactor) Activity(days []time.Time) ([]DayActivity, error) {
	idx, err := c.loadIndex()
	if err != nil {
		return nil, err
	}
	...
	for _, d := range days {
		...
		if idx != nil {
			if row, ok := idx.days[key]; ok {
				out = append(out, DayActivity{Day: key, Turns: int(row.Turns), CostUSD: row.CostUSD})
				continue
			}
		}
		...
		activity := DayActivity{Day: key, Turns: len(turns)}
		for _, t := range turns {
			activity.CostUSD += t.CostUSD
		}
