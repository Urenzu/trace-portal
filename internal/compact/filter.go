...
//	projectid:9f2c1d8a       one exact project, as a drill-in link writes it
...
func ParseFilter(q string) Filter {
	...
	for _, raw := range splitTerms(q) {
		...
		if name, rest, ok := strings.Cut(raw, ":"); ok && rest != "" {
			...
			case "project", "projectid", "branch", "model", "tool", "id", "has", "cost", "turns":
...
func (t term) match(s query.Session) bool {
	...
	case "projectid":
		// Exact, unlike everything else here: this term is generated by a link
		// rather than typed, and a substring match could pull in a second
		// project whose digest happens to share a prefix.
		return strings.EqualFold(s.ProjectID, t.value)
	case "branch":
